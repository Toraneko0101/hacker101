...
??? success
    ...
    ```text
    中心C(𝕔)から円周上までの大きさを半径rとしたとき
    円は中心からの距離が一定である点の集合に当たるので
    点Pまでの位置ベクトルを𝕡とすると、
    |𝕡 - 𝕔| = rを満たすような任意の点は、円の要素となる
    
    ----------------

    任意の点P(x,y)が円周上に存在するためには
    Pの位置ベクトル𝕡が、|𝕡 - 𝕔| = rを満たす必要がある

    [使用例1] 点Pが与えられたとき、それが集合上の点か判別できる

    [使用例2] 条件を満たす点を集めることで、円が表現できるならば
             それは円を示していることと同義
    ...
    円周角の定理より、∠APB = 90°
    ∠APB ≠ 90°であるとき、点Pは円周上に存在しないため 
    AP⊥BPであるような任意の点集合が、円となる
    つまり、<𝔸ℙ, 𝔹ℙ> = 0

    よって、<(𝕡 -𝕒),(𝕡 -𝕓)> = 0は円のベクトル方程式になる
    ```
