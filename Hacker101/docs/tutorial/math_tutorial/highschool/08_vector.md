...
??? success
    ...
    [証明]
      ...
      なお、3つ目の成分について平面と直線全体を回転させることで
      任意の平面と直線について示せるため、一般性は失われない
      ...
      ここで、平面α上の任意の直線の方向ベクトルを𝕧 = (x_1, x_2, 0)とする
      ...
      𝕧 = k_1𝕧_m + k_2𝕧_n
      = k_1(a_1, a_2, 0) + k_2(b_1, b_2, 0)
      = (k_1a_1 + k_2b_1, k_1a_2 + k_2b_2, 0) 

      ここで、<𝕧_l, 𝕧> = 0であれば、直線lの方向ベクトルと、
      平面α上の任意の直線の方向ベクトルは垂直であるといえる

      <𝕧_l, 𝕧> 
      = c_1(k_1a_1 + k_2b_1) + c_2(k_1a_2 + k_2b_2) + 0 
      = k_1(c_1a_1 + c_2a_2) + k_2(b_1c_1 + b_2c_2)
...
## Q26 平面の方程式
    ...
    ※見方を換えれば法線ベクトルが(a,b,c)であるような
      平面の方程式はax + by + cz + d = 0と考えられる
    ```
