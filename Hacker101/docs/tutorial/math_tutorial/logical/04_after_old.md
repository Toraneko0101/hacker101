...
??? success
    ...
    Y^Xという表し方について
      ・X = {a_1, ..., a_m}
      ・Y = {b_1, ..., b_n}という有限集合について考える
      ・この時、写像について考えるとその個数は
        a_1と対応可能なYの元はn個, a_2と対応可能なYの元もn個...
        となることから、n^m個となる
      ・つまるところ、集合から考えると写像の個数は|Y^X| = |Y|^|X| 
    ...
    冪集合の2について
      ・集合Xから、2個の要素{0,1}からなる集合への写像をf:X --> {0,1}を
      　すべて集めてできる集合{0,1}^Xと、
      　Xの冪集合2^X = {A | A⊆X}について考える

      ・この時、写像集合{0,1}^X --> 冪集合2^Xは全単射(1対1対応)になる
  
      ・任意の写像f: X --> {0,1}に対して、Xの部分集合A = {x∊X | f(x) = 1}と
        定めれば、冪集合2^Xの2が{0,1}を指していることがよくわかる。
        ※fによって、Xの各要素はAに属するか属しないか決まるので
          冪集合の各要素は、fの解釈と対応する
    ```
    ...
    fによるAの像
      ・写像f:X-->Yと、Xの部分集合Aに対して
        f(A) = {f(x) | x∊A}
        --> 集合Aをfでうつした、Yの部分集合
    
    fによるAの逆像
      ・写像f:X-->Yと、Yの部分集合Bに対して
        f^{-1}B = {x∊X | f(x)∊B}
        --> 写像fでうつした先がBの要素に対応するような、Xの部分集合

    ※像と値域
      ・像は定義域の特定の部分集合を写像の対象としていて
      ・値域は、定義域全体を写像の対象としているf(X) = {f(x) | x∊X}
      ・終域は、f:X-->Yとしたときの、集合Y

    ※具体例
      f: R --> R>=0 を、f(x) = x^2とする
      この時、Xの部分集合A = [1,2]とすると
      f([1,2]) = [1,4]

      また、B = [1,4]⊂Yの部分集合を考えると
      f^{-1}([1,4]) = [-2, -1] ∪ [1, 2]
                    = {x∊R | -2≦x≦-1, 1≦x≦2}
    ...
    ```text
    単射
      ・集合Xを定義域、集合Yを終域とし、x,x'を定義域の任意の元とする
      ・x≠x'ならば、f(x)≠f(x')となる時、fは単射
      ・対偶：f(x)=f(x')ならば、x=x'の方が、わかりやすかったりする
      ・y∊Yの逆像f^{-1}(y)は高々1つの要素しか持たない
        --> 写像fで写した先がyになるXの要素は1つだけ

      ・∀x,y∊A[x≠y ⇒ f(x)≠f(y)]
      ・∀x,y∊A[f(x)=f(y)⇒ x=y]

    全射
      ・どのy∊Yに対しても、f(x) = yという、x∊Xが1つ以上存在するとき、fは全射
      ・f(X)=Y。つまり、終域と値域が一致する

    全単射
      ・全射であり、単射な写像
      ・Xの全要素とYの全要素の間に、1対1の対応が為されている状態
      ・全単射f:X-->Yに対しては、逆写像f^{-1}:Y --> Xを定めることができる

    合成写像
      ・2つの写像f:X-->Y, g:Y-->Zの合成写像g〇f:X-->Zについて考える
      ・g〇f(x) = g(f(x))
      ・g〇f:x|--> g(f(x)) (※元と値の関係性)
    ...
    恒等写像
      ・集合Xの各元xに対して、同じ元xを対応させる写像id_x: X --> X
      ・任意の写像f:X-->Yに対して、f〇id_x = id_y 〇 f = f
        --> id_x(x) = xなので、
        --> (f〇id_x)(x) = f(id_x(x)) = f(x)
        --> (id_y〇f)(x) = id_y(f(x)) = f(x)
        --> 恒等写像を合成しても、元の写像は変わらない
    ```
    ...
    ```text
    X上のn項関係
      ・X^nの部分集合をRで表す。この時集合Xに対して、
      　X^nの部分集合R(その特性関数は、χ_R:X^n --> {0,1})をX上のn項関係という
        --> X^n: Xからn個の要素をとってくる(重複可能)としたときの、直積集合
        --> R  : 直積集合のうち、関係性が成立しているn個の要素の組をすべて集めたもの
                 実際に組み合わせがRの元にあれば1になる
        --> この時、対応する特性関数をX上のn項述語という       
      ・特にn=2の時、Xの要素の組(a,b)がRに属すことを、aRbと表す

    ※n項関係
      ・要素同士がある条件を満たしているかどうかを考える際に、
        具体的な要素のペアや、n個の組み合わせを扱う

        --> X^nの部分集合Rについて、X^nの各元がRに属しているかいないか考えれば
            自然とX上のn個の要素で構成された集合が、ある条件を満たしているか
            考えることになる
    
    ※n項述語
      ・n個の項を引数に取り、条件が真か偽を返す関数
    
    例)
    
      [1] 任意の集合Xに対して、要素間の等号a = bは、X上の２項関係
      [2] a ≡ b mod nという関係を考える。これは整数全体の集合Z上の2項関係
      [3] 写像f:X-->Yが与えられたとき、Xの要素a,bにたいして、f(a)=f(b)となるとき、
        a≡bと書くことにすると、≡はX上の2項関係
      [4] 集合の包括関係A⊆Bは、Xの冪集合2^X上の2項関係
      [5] 大小関係a≦bは、実数の集合R上の2項関係

    同値関係
      ・集合X上の2項関係Rが次の3つの条件を満たすとき、X上の同値関係であるという

      1 反射律：任意のa∊Xに対して、aRa
      2 対称律：任意のa,b∊Xに対して、aRbと、bRaは同値
      3 推移律：任意のa,b∊Xに対して、aRbかつ、bRcなら、aRc

      [1]の場合
        aRa : aとaの間に、等号関係が成立している --> a = a
        aRbとbRaが同値 --> a = b ⇔ b = a
        aRb∧bRc⇒aRc  --> a = b ∧ b = c ⇒ a = c
        --> 任意の集合Xに対する、要素間の等号a = bは、X上の同値関係
    
    順序(半順序ともいう)
      ・反射律と、推移律は同値関係と同じ
      ・反対称律：任意のa,b∊Xに対して、aRbかつbRaならばa = b
    
      [5]の場合
      aRa --> a≦a
      aRb∧bRc⇒aRc --> a≦b∧b≦c⇒a≦c
      aRb∧bRa⇒a=b --> a≦b∧b≦a⇒a=b
    ```
    ...
    集合X上のn項演算
      ・X^nからXへの写像
      ・直積集合の各元が、写像により、X内の任意の元に移される
      --> 直積集合X^nは、Xのn個の要素(同じ要素を含んでもよい)を並べた順序対から成り立つ
    ...
    分配束
      ・集合L上の二項演算∨と、∧の組が以下を満たすとき
        代数系<L;∨,∧>を分配束という
      
      ※代数系：集合Lに対して、演算aが定義され、かつ
               集合Lが演算aに対して閉じているとき、集合と演算の組を代数系といい
               <L;a>のようにあらわす

      交換則：x∧y = y∧x, x∨y = y∨x
      結合則：(x∧y)∧z = x∧(y∧z), (x∨y)∨z = x∨(y∨z)
      吸収則：(x∧y)∨x = x,  (x∨y)∧x = x
      分配測：(x∨y)∧z = (x∧z)∨(y∧z), (x∧y)∨z = (x∨z)∧(y∨z)
    
    ブール束(ブール代数)
      ・さらにLの特別な元0,1と、単項演算￢について、以下が成り立つとき、
        代数系<L;∨,∧,0,1,￢>をブール束(可補分配束)という
      
      補元則：x∨￢x = 1,  x∧￢x = 0
      単位元の存在：A∧1 = A, A∨0 = A
      零元(どんな元との演算も零元にする)の存在：A∨1 = 1, A∧0 = 0

    論理演算との違い
      ・論理演算：命題を操作対象とする。背後にある法則や性質は
                 ブール代数で定義される法則(交換律、結合律、分配率、補元律等に従う)
      ・ブール代数：論理演算をより抽象的に扱う
                   命題に限らず、集合や回路設計などの他の構造に対しても使われる
    
    ブール代数の例
      ・X={a,b}の冪集合 = {φ, {a}, {b}, {a,b}}
