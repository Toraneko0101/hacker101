...
??? success
    ...
    3 power-on self-test(POST)を実行
      3-1 UEFIのコード自体に問題がないかチェック
      3-2 ハードウェアを検出（起動に必要なハードウェアを
      　　検出できない場合、致命的なエラーを吐く）
      3-3 起動に必要なハードウェアを初期化
      3-4  全ての起動可能なストレージデバイスをスキャン
      3-5 有効なGUIDパーティションテーブル(GPT)を特定

    4 ファームウェアはGPTをスキャンして、EFIシステム
      パーティション(ESP)を見つける

    5 ESP内のブートエントリを使用して、ブートローダを発見

    6 ESP内のブートローダをメモリにロード

    7 ブートローダ(GRUB2)が、カーネル(vmlinuz)と
      initramfsをメモリにロードする
    ...
    10 カーネルが、initramfsを仮のルートファイルシステム
      としてマウントする

    11 initramfsのinitからsystemdを起動する

    12 ディスク内のルートファイルシステムをマウント

    13 本物のルートファイルシステムに切り替え

    14 /sbin/init(PID:1)を実行し、起動シーケンスを開始

    15 default.targetに記述されている依存関係を満たすように
